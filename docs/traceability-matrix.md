...
3. **NFR‑006** — architecture + pagination in place (adding an entity/village needs no schema change), now **load-tested at pilot volume** (Artillery, `load-test/`: 50 orgs/500 users → sustained 77 req/s, p95 ≈ 211 ms, p99 308 ms, 0 errors — within the < 500 ms budget), so caching is not required at pilot scale. Production capacity planning (horizontal scaling, soak, Week 2/3 pipeline endpoints) is follow-up.
...
|--------|-------|--------------|
| ✅ Done | 10 | RBAC‑001, FR‑010, FR‑007, FR‑Add‑03, FR‑Add‑02, NFR‑003, NFR‑006, NFR‑013, NFR‑015, NFR‑009 (frontend) |
| 🟡 Partial | 2 | NFR‑001 (at‑rest = deployment‑delegated), NFR‑004 (decisions = R7) |
...
- **NFR‑006:** ✅ load-tested at pilot volume (`load-test/`, 77 req/s, p95 211 ms, 0 errors); remaining follow-up is production capacity planning — horizontal scaling, pool sizing, a soak run, and load against the Week 2/3 pipeline endpoints.
