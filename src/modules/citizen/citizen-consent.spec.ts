...
const VERIFIED_CHALLENGE = {
  ...
  consumedAt: null,
  // GAP-10: submitResponse re-validates expiry at submission, not just at
  // verifyOtp — must still be in the future here for the happy-path tests.
...
function makeService(overrides: { activeNotice?: unknown; noticeError?: Error } = {}) {
  ...
  const tx = {
    ...
    // GAP-04: submitResponse enqueues the scoring job on the same
    // transaction via a raw SQL call to graphile_worker.add_job.
  ...
  const audit = {
    ...
    // GAP-11: the response audit event is written on the SAME transaction
    // (recordWithTx), not the fire-and-forget record() — see
    // CitizenService.submitResponse.
    recordWithTx: vi.fn(async (_tx: unknown, _entry: Record<string, unknown>) => undefined),
  };
...
describe('CitizenService.submitResponse — consent (RIO-NFR-002)', () => {
  ...
  it('logs the consent version, and still never the respondent contact details', async () => {
    ...
    const entry = audit.recordWithTx.mock.calls[0]![1];
