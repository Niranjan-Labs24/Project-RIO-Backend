...
export class ScoreRollupService {
  ...
  async recalculateStudyScores(studyId: string, surveyId: string): Promise<void> {
    await this.tenant.runInOrgContext(async (tx) => {
      // Find all response IDs for this study
      ...
      for (const v of distinctVillages) {
        await this.calculateRollups(studyId, surveyId, v);
      }
      await this.calculateRollups(studyId, surveyId, null);
    });
   ...
   */
  async calculateRollups(studyId: string, surveyId: string, villageId: string | null): Promise<void> {
    await this.tenant.runInOrgContext(async (tx) => {
...
// Helper to get active org id from tenancy context (safe fallback for seed/test execution)
function currentOrgId(): string {
  try {
    const { getOrgStore } = require('../../tenancy/org-context');
    return getOrgStore()?.orgId || "00000000-0000-0000-0000-000000000000";
  } catch (e) {
