...
// KNOWN GAP (sprint1-integration + sprint3 merge, 2026-09-16) — the reviewer-SLA
// report section shipped on sprint1-integration before the Arabic i18n
// catalogue (sprint3) existed, so the frontend's app.reports.content has never
// seen this heading. It prints in English in an Arabic export until someone
// with edit access to Project-RIO-Frontend/messages/{en,ar}.json adds a key for
// it and `pnpm sync:report-labels` is re-run — do not "fix" this by hand-editing
// report-labels.generated.ts (see its own DO-NOT-EDIT header).
const KNOWN_UNTRANSLATED_HEADINGS = ['Reviewer Open-Queue Health'];
...
describe('report-doc.ts heading coverage', () => {
  ...
  it('resolves every hardcoded heading to a catalogue key', () => {
    const unresolved = headings.filter(
      (h) => keyForEnglish(h) === null && !KNOWN_UNTRANSLATED_HEADINGS.includes(h),
    );
    expect(
      unresolved,
